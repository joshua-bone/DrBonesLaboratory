...
this.Load=function(this)
  this.Resource=LoadTexture(this.path.."stopwatch.bmp")
  this.glist=glGenLists(2)
  local h=1/256
  glNewList(this.glist,GL_COMPILE)
	  glBindTexture(GL_TEXTURE_2D,this.Resource)
    RenderPlaneUV(0.5,0.5,BOTTOMLEVEL,0,0+h,1,1/2-h) --mask
  glEndList()
  glNewList(this.glist+1,GL_COMPILE)
	  glBindTexture(GL_TEXTURE_2D,this.Resource)
    RenderPlaneUV(0.5,0.5,BOTTOMLEVEL,0,1/2+h,1,1-h) --bomb
  glEndList()
end
