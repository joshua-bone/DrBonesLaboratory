...
AskLoadMap=function()
  if CheckDirty() then
    local name=GetOpenFileName("Open Map","lua *.lua\0*.lua\0All *.*\0*.*\0")
    if name~=nil then 
      LoadMap(name)
      SetTitle(name.." | Puzzle Studio")
      FileName=name
      return true
    end
  end
...
AskSave=function()
  if level~=nil then
    local name=GetSaveFileName("Save map","lua *.lua\0*.lua\0All *.*\0*.*\0")
    return SaveMap(name)
  end
  return true
end
...
SaveReplay=function()
  if replay~=nil then
    ...
    if name~=nil then
      ...
      file:close()
      PlayHud[2][2][3]="Replay saved" -- indicate saved
    end
  end
end
...
--EXPERIMENTAL FOR GAMEPLAY--
  ...
  local clipx,clipy=level.clip[1],level.clip[2]
...
-----------------------------
  local startz=math.max(gamestate.CurrentDeck-3,1)
...
function inctop()
  if #map[1]>=64 then return end
  local x,z
  for x=1,#map do table.insert(map[x],1,{})
    for z=1,#map[2][2] do 
      local space="8006ef05-0bdb-4d6a-bb8c-f7533439e091"
      table.insert(map[x][1],1,{})
      ListAppend(map[x][1][z],library[space])
...
function incleft()
  ...
  local sy=#map[1]
  table.insert(map,1,{})
  for y=1,sy do map[1][y]={} PaintWith(Parts.right.item,1,y) end
  Editor.camera.x=Editor.camera.x+1
end
...
function incright()
  ...
  local y
  table.insert(map,{})
  local sx,sy=#map,#map[1]
  for y=1,sy do map[sx][y]={} PaintWith(Parts.right.item,sx,y) end
end

function incbottom()
  if #map[1]>=64 then return end
  local x
  local sx,sy=#map,#map[1]+1
  for x=1,sx do map[x][sy]={} PaintWith(Parts.right.item,x,sy) end
end

ToggleDimensions=function()
  if #Editor.Hud[1][2]~=0 then
     Editor.Hud[1][2]={}
     Editor.Hud[2][1]={}
     Editor.Hud[2][3]={}
     Editor.Hud[3][2]={}
    ...
    Editor.Hud[1][2]={
      MakeClickableText("y+","yellow","green",inctop,nil),
      MakeClickableText("y-","yellow","green",dectop,nil),
    }
    Editor.Hud[2][1]={
      MakeClickableText("+","yellow","green",incleft,nil),
      MakeClickableText("-","yellow","green",decleft,nil),
    }
    Editor.Hud[2][2]={
      MakeClickableText("+","yellow","green",incabove,nil),
      MakeClickableText("-","yellow","green",decabove,nil),
      MakeClickableText("+","yellow","green",incbelow,nil),
      MakeClickableText("-","yellow","green",decbelow,nil),
    }
    Editor.Hud[2][3]={
      MakeClickableText("y+","yellow","green",incright,nil),
      MakeClickableText("y-","yellow","green",decright,nil),
    }
    Editor.Hud[3][2]={
      MakeClickableText("+","yellow","green",incbottom,nil),
      MakeClickableText("-","yellow","green",decbottom,nil),
    }
  end
end
...
CheckReplay=function()
  local idx=2
  if replay~=nil then
     PlayHud[3][2][2]=MakeClickableText("Start Replay","yellow","green",StartReplay,nil)
     idx=3
  end
  PlayHud[3][2][idx]=MakeClickableText("Exit","yellow","green",function() stop() end,nil)
end
...
-- run the game
  ...
  if level.time~=nil then
    if level.time~=0 then
      ...
      PlayHud[1][1]["time"]="Time:"..math.floor(level.time/60)
      if level.time==0 then
        lose()
      end
    end
...
function RemoveFromGame(obj)
  ...
  if obj.a.player==true then lose(obj.deathmessage) end
end
