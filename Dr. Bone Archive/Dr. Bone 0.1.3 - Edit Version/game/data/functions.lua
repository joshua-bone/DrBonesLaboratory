...
-----------------------------
  ...
  glTranslatef(1-showx,showy-1,-gamestate.camera.z-gamestate.CurrentDeck) --translates back to (1, 1)
  --glTranslatef(math.max(1, math.floor(showx-clipx))-showx, showy-math.max(1, math.floor(showy-clipy)), -gamestate.camera.z-gamestate.CurrentDeck)
  for z=startz,gamestate.CurrentDeck do
    ...
    for x=1,#map do
	--for x=math.max(1, math.floor(showx-clipx)), math.min(math.floor(showx+clipx), sizex) do
      local dx=x-showx
      if dx>=px then break end
      if dx>-px then
        glPushMatrix()
        for y=1,#map[x] do
		--for y=math.max(1, math.floor(showy-clipy)), math.min(math.floor(showy+clipy), sizey) do
          local dy=y-showy
