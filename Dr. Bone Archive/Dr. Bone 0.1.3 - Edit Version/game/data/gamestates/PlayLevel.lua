...
PlayLevel.render=function()
  if PlayLevel.renderThisTurn==3 then
  
	  PlayLevel.renderThisTurn=1
	  --beta FPS testing
	  if not PlayLevel.countframes then PlayLevel.countframes=0 PlayLevel.lastclock=os.clock() end
	  PlayLevel.countframes=PlayLevel.countframes+1
	  ...
	  --beta FPS testing
	  if PlayLevel.countframes==60 then
		PlayLevel.countframes=0
		PlayLevel.fps=60/(os.clock()-PlayLevel.lastclock)
		PlayLevel.lastclock=os.clock()
	  end
	  
  end
  PlayLevel.renderThisTurn = PlayLevel.renderThisTurn + 1
end
...
{{},{},{}},
{{},{},{function() return string.format("%4f", PlayLevel.fps).." FPS" end}},
}